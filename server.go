...
func NewServer(opts ...ServerOption) *Server {
	...
	// append plugin if exist
	for _, pluginName := range s.opts.pluginNames {
		if plugin, in := plugin.PluginMap[pluginName]; in {
...
func (s *Server) Serve() {
	...
	signal.Notify(ch, syscall.SIGTERM, syscall.SIGINT, syscall.SIGQUIT, syscall.SIGSEGV, syscall.SIGKILL)
	sig := <-ch
	fmt.Println("Close sig | ", sig)
	s.Close()
}
